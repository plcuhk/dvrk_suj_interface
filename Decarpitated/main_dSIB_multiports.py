...
class MyMainWindow(QtWidgets.QWidget):
    ...
    def slotThread12(self, text): # show value
        ...
        # valueText = '{:.3f}'.format(value *2.5 / 16777215)   # Original display of voltage
        Mode = ['SUJ1', 'SUJ2', 'ECM']
        current_mode = 'SUJ1'
        voltage = value / 16777215 * 2.5

        if current_mode == 'SUJ1' or current_mode == 'SUJ2':
            if channel == "0":
                if current_mode == 'SUJ1':
                    valueText = '{:.3f}'.format((voltage - 0.115) * 0.460422)
                else:
                    valueText = '{:.3f}'.format((voltage - 0.109) * 0.462567)
                self.ui.lcdNumber_1.display(valueText)
            elif channel == "1":
                valueText = '{:.1f}'.format((voltage / 2.5 * 2 * pi - pi) * rad2deg )
        ...
        elif current_mode == 'ECM':
            if channel == "0":
                valueText = '{:.3f}'.format((voltage - 0.141) * 0.440917 )
                self.ui.lcdNumber_1.display(valueText)
            elif channel == "1":
                valueText = '{:.1f}'.format((voltage / 2.5 * 2 * pi - pi) * rad2deg )
                self.ui.lcdNumber_2.display(valueText)
    ...
    def link(self):
        ...
        if len(self.plist) <= 0: # 无串口
            ...
            for port in list(self.plist):
                ...
                readings = self.serial.read_until()
                print(readings)
                self.serial.close()
    ...
    def run(self):
        try:
            while self.flag1 is "start":
                ...
                # print(readings)
                test_sum = 0
                for num in readings[0:-1]:
                    num = num.decode('utf-8')
                    test_sum += int(num[-3], 16)
